      ...
      <div className="bg-surface-900 border border-surface-700/50 rounded-2xl shadow-2xl max-w-lg w-full mx-4 animate-scale-in overflow-hidden">
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <h2 className="text-lg font-semibold text-surface-100">{t('updateDialog.updateAvailable')}</h2>
              <p className="text-sm text-surface-400">{t('updateDialog.readyToInstall', { version: updateInfo.version })}</p>
            </div>
          </div>
        </div>
        ...
        {renderedBody && (
          <div className="px-6 pb-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-surface-500 uppercase tracking-wider">{t('updateDialog.releaseNotes')}</span>
            </div>
            <div
              className="bg-black/20 rounded-xl p-4 max-h-64 overflow-y-auto custom-scrollbar text-sm text-surface-300 leading-relaxed update-markdown"
        ...
        <div className="px-6 pb-4">
          <label className="flex items-center gap-2.5 cursor-pointer group" onClick={() => setDontAskAgain(!dontAskAgain)}>
            ...
            <span className="text-xs text-surface-400 group-hover:text-surface-300 transition-colors">{t('updateDialog.dontAskAgain')}</span>
          </label>
        </div>
